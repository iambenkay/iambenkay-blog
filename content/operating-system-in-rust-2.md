...
PT_LOAD = "an ELF loader should make this segment exist in memory."
FLAGS(...) bits: 4 = R, 5 = R+X, 6 = R+W.
So you end up with three logical regions: the stack (RW), the code+rodata (RX), and the mutable data + BSS (RW). Each section below is assigned to one of these via the trailing :segment_… syntax.
